...
const LanguageSwitcher = ({ classes, svgColor }: IProps) => {
  const { i18n } = useTranslation();
  const { currentLanguage, changeLanguage } = useData();
    ...
    <>
      <Button intent={"outline"} onClick={toggleLanguage}>
        <div className={`flex gap-1.5 p-2  font-medium ${classes} `}>
          <span className="text-bolder text-xl">
            {currentLanguage === "en" ? "العربية" : "English"}
          </span>
          <span className={`${svgColor}`}>
            ...
            >
              <g id="Linear / Map & Location / Global">
                ...
                <path
                  ...
                  stroke={"#131312"}
                />
                <path id="Vector_3" d="M2 12H22" stroke={"#131312"} />
              </g>
            </svg>
          </span>
        </div>
